...
@description('Enable provisioning of a new Azure OpenAI resource. If false, provide externalOpenAiEndpoint.')
param enableOpenAI bool = false

@description('Existing Azure OpenAI endpoint to use when enableOpenAI is false')
param externalOpenAiEndpoint string = ''

@description('Enable monitoring resources (Log Analytics + Application Insights)')
...
var chatModel = {
  modelName: !empty(chatModelName) ? chatModelName : startsWith(openAiHost, 'azure') ? 'gpt-5-mini' : 'gpt-5-mini'
  deploymentName: !empty(chatDeploymentName) ? chatDeploymentName : 'gpt-5-mini'
  deploymentVersion: !empty(chatDeploymentVersion) ? chatDeploymentVersion : '2025-04-01-preview'
...
module appServicePlan './core/host/appserviceplan.bicep' = {
  ...
  params: {
    ...
    location: location
    tags: tags
    // Choose plan SKU based on hosting type
    sku: azFunctionHostingPlanType == 'flexconsumption'
      ? {
          name: 'FC1'
...
module api './app/api.bicep' = {
  ...
  params: {
    ...
    aiServiceUrl: enableOpenAI ? ai.outputs.endpoint : externalOpenAiEndpoint
  }
}

module ai 'core/ai/openai.bicep' = if (enableOpenAI) {
  name: 'openai'
  scope: rg
  params: {
...
// Allow access from api to storage account using a managed identity
module cogRoleAssignmentApi 'app/ai-Cog-Service-Access.bicep' = if (enableOpenAI) {
...
module cogRoleAssignmentUserIdentityApi 'app/ai-Cog-Service-Access.bicep' = if (enableOpenAI) {
...
module monitoring './core/monitor/monitoring.bicep' = if (enableMonitoring) {
  name: 'monitoring'
  scope: rg
  params: {
    location: location
...
module appInsightsRoleAssignmentApi './core/monitor/appinsights-access.bicep' = if (enableMonitoring) {
...
output AZURE_OPENAI_ENDPOINT string = enableOpenAI ? ai.outputs.endpoint : externalOpenAiEndpoint
