    ...
    try:
        ...
        if user_id:
            ...
            try:
                ...
                pass
            # Filter tools based on allowed_tools if provided
            try:
                ...
                if normalized_tools is not None and responses_args.get("tools"):
                    filtered_tools = []
                    ...
                    logging.info(f"[ask] Filtered tools based on allowed_tools {normalized_tools}: {len(filtered_tools)} tools remaining")
            except Exception as e:
                logging.warning(f"[ask] Failed to filter tools: {e}")
            ...
            if responses_args.get("tools"):
                tool_context = {"user_id": internal_user_id} if internal_user_id else None
                output_text, response = run_responses_with_tools(client, responses_args, tool_context=tool_context)
                logging.info(f"[ask] After run_responses_with_tools: output_text length={len(output_text or '')}, has_text={bool(output_text)}")
                if not output_text:
                    # Check if tools were actually used - if so, don't fallback without tools
                    tools_used = getattr(response, "_classic_tools_used", [])
                    if tools_used:
                        # Tools were used but no final text - force a synthesis call
                        logging.info(f"[ask] Tools were used {tools_used} but no final text, forcing synthesis")
                        try:
                            # Create a simple synthesis prompt with the original question
                            last_user_msg = None
                            for msg in reversed(responses_args.get("input", [])):
                                if msg.get("role") == "user":
                                    ...
                                    break
                            if last_user_msg:
                                # Create generic synthesis prompt based on tools used
                                tool_names = [t.get("name", "tool") for t in tools_used]
                                tools_str = ", ".join(tool_names) if len(tool_names) > 1 else tool_names[0] if tool_names else "tool"
                                synthesis_prompt = f"The {tools_str} tool has provided search results. Please analyze these results and provide a clear, direct answer to the user's question: '{last_user_msg}'. Do not just list links - extract the actual information and present it in a useful format. If it's a weather question, provide the current temperature, conditions, and forecast. If it's about templates or files, summarize what was found."
                                synthesis_args = {
                                ...
                                synthesis_response = client.responses.create(**synthesis_args)
                                output_text = getattr(synthesis_response, "output_text", None) or ""
                        except Exception as e:
                            logging.warning(f"[ask] Synthesis call failed: {e}")
                    else:
                        # No tools used - fallback without tools is appropriate
                        try:
